...
function buildPgSchemaTs(params: {
  ...
  for (const tableName of tables) {
    ...
    const tableDecl = isNonPublicSchema
      ? `${params.schema}Schema.table("${tableName}", {`
      : `pgTable("${tableName}", {`;
    lines.push(`export const ${ident} = ${tableDecl}`);
    for (const col of tableColumns) {
      const propName = safeIdentifier(col.column_name);
      const pgType = normalizeType({
        dataType: col.data_type,
...
function buildSqliteSchemaWrapperTs(params: {
  ...
  const lines: string[] = [];
  lines.push(createHeader({ schema: params.schema }));
  lines.push(`export * from ${JSON.stringify(params.sqliteSchemaImportPath)};`);
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push("/**");
  lines.push(" * Client-side outbox used by the oosync runtime.");
  lines.push(" */");
  lines.push("export const syncPushQueue = sqliteTable(");
  lines.push('  "sync_push_queue",');
  lines.push("  {");
  lines.push('    id: text("id").primaryKey().notNull(),');
  lines.push('    tableName: text("table_name").notNull(),');
  lines.push('    rowId: text("row_id").notNull(),');
  lines.push('    operation: text("operation").notNull(),');
  lines.push('    status: text("status").default("pending").notNull(),');
  lines.push('    changedAt: text("changed_at").notNull(),');
  lines.push('    syncedAt: text("synced_at"),');
  lines.push('    attempts: integer("attempts").default(0).notNull(),');
  lines.push('    lastError: text("last_error"),');
  lines.push("  },");
  lines.push("  (t) => [");
  lines.push(
    '    index("idx_push_queue_status_changed").on(t.status, t.changedAt),'
  );
  lines.push('    index("idx_push_queue_table_row").on(t.tableName, t.rowId),');
  lines.push("  ]");
  lines.push(");");
  lines.push("");
  return lines.join("\n");
...
function buildWorkerEntrypointTs(params: {
  ...
  const lines: string[] = [];
  lines.push(createHeader({ schema: params.schema }));
  lines.push('import createWorker from "oosync/worker";');
  lines.push(
  ...
  lines.push("");
  lines.push("export default createWorker({");
  lines.push("  schemaTables,");
  lines.push("  syncableTables: SYNCABLE_TABLES,");
  lines.push("  tableRegistryCore: TABLE_REGISTRY_CORE,");
  lines.push("  workerSyncConfig: WORKER_SYNC_CONFIG,");
  lines.push("});");
  lines.push("");
  return lines.join("\n");
}
...
async function main(): Promise<void> {
  ...
  const outputSqliteSchemaWrapperFile = config.outputs?.sqliteSchemaWrapperFile
  ...
  const outputWorkerEntrypointFile = config.outputs?.workerEntrypointFile
